...
class Line:
    ...
    def __init__(self, evolution_time, time_units='s', conduction=True, convection=True, settling_mode='stokes terminal',
        ...
        self.materials = {}
        self.conductivities = []
        if "nusselt_evolution.csv" in os.listdir(os.getcwd()):
            os.remove("nusselt_evolution.csv")
        self.nusselt_evolution = ({
            'time': []
        })
    ...
    def to_csv(self):
        """
        Write the mesh/nusselt evolution dataframes to a csv.
        :return:
        """
        console.event("mesh.csv now available in {}!".format(os.getcwd()), verbose=self.verbose)
        # write the mesh dataframe to a csv file
        self.mesh.to_csv("mesh.csv", index=False)
        # write the nusselt evolution dictionary to a csv file
        pd.DataFrame.from_dict(self.nusselt_evolution).to_csv("nusselt_evolution.csv", index=False)
    ...
    def get_nusselt_evolution(self):
        """
        Returns the nusselt number evolution dataframe.
        :return:
        """
        return self.nusselt_evolution
    ...
    def update(self, auto_update=True, timestep=False):
        ...
        while auto_update is True and self.evolution_time > 0:
            ...
            nusselt = heat.nusselt(layers=self.layers, mesh=self.mesh, spatial_res=self.spatial_res,
                                   model_time=self.evolution_time)

            # track nusselt number evolution over time for each layer
            self.nusselt_evolution['time'].append(self.evolution_time)
            for layer in nusselt:
                if layer not in self.nusselt_evolution.keys():
                    self.nusselt_evolution.update({layer: [nusselt[layer]]})
                else:
                    self.nusselt_evolution[layer].append(nusselt[layer])
